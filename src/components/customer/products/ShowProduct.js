...
const ShowProductPage = () => {
    ...
    useEffect(() => {
        ...
        initBiddersCountUp()
        fetchProduct()
        const intervalId = setInterval(fetchProduct, 50000);
        return () => {
    ...
    function handleFullPayment() {
        ...
        apiPost("/orders/full-payment", {productId: id}).then((response) => {
            fetchProduct()
        }).catch((error) => {
        }).finally(() => {
    ...
    async function fetchBidHistory() {
        apiGet(`/orders/product/${id}`).then((response) => {
    ...
    function onSubmit(event) {
        ...
        setLoading(true)
        setBidError(null)
        apiPost("/orders", {"productId": product.id, "amount": bidAmount})
            .then((res) => {
                fetchProduct()
            }).catch((error) => {
            if (error.response.status === 412) {
                setBidError("Oops! It seems your account balance is too low to place a Order. To keep participating, top up your balance now. Make a deposit and keep the excitement going!")
            }
            if (error.response.status === 406) {
                setBidError("Oops! Your Order is Below the Highest Order. Probably due to a previous concurrent Order!")
            }
        }).finally(() => {
            setLoading(false)
        })
                ...
                <div className="card p-5 mb-5">
                    <div className="card-body p-0">
                        ...
                        <div className="mt-5">
                            ...
                            <div className="col-md-12 mt-5">
                                <h6 className="fs-4 text-muted">Expiration Date</h6>
                                <p className="fs-3">{(new Date(product.expirationDate)).toLocaleString('en-us', {
                                    weekday: "long",
                                    year: "numeric",
                                    month: "short",
                                    day: "numeric",
                ...
                <div className="card mb-5">
                    <div className="card-body">
                        ...
                        {product.status === "Sold" && product.highestBidUser.id === currentUser.id && <>
                            ...
                            {product && product.orderPaymentDueDate && product.status === "Sold" &&
                                <CountDown value={product.orderPaymentDueDate}/>}
                        </>}
                        ...
                        <form action="" onSubmit={onSubmit}>
                            <div className="col-md-12 fv-row mb-5">
                                ...
                                {product.highestBidAmount === 0 && <div className="mb-5">
                                    <span className="text-muted text-start fs-5 fw-semibold">Be the first to Order this product</span>
                                </div>}
                            ...
                            <button disabled={loading} type="submit"
                                ...
                                {!loading && <span className='indicator-label'>Order Now</span>}
                                {loading && (
                                    <span className='indicator-progress' style={{display: 'block'}}>
                                        Submitting...
                                        <span className='spinner-border spinner-border-sm align-middle ms-2'></span>
